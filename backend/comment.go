...
func createCommentHandler(w http.ResponseWriter, r *http.Request) {
	...
	var postOwnerID int
	var privacy string
	err = db.QueryRow("SELECT user_id, privacy FROM posts WHERE post_id = ?", postID).Scan(&postOwnerID, &privacy)
	if err != nil {
	...
	if userID == postOwnerID {
		...
		case "public":
			allowed = true
		case "almost_private":
			err = db.QueryRow("SELECT 1 FROM followers WHERE follower_id=? AND following_id=?", userID, postOwnerID).Scan(new(int))
			if err == nil {
				allowed = true
			}
		case "private":
			err = db.QueryRow("SELECT 1 FROM post_allowed_followers WHERE post_id=? AND follower_id=?", postID, userID).Scan(new(int))
			if err == nil {
				allowed = true
			}
		}
